...
class IndustryController extends Controller
{
    ...
    public function industries_save(Request $req){
        ...
        $sql = "INSERT INTO `nganhhang`(`MA_NGANH`, `TEN`) VALUES (:MA_NGANH,:TEN)";
        $param =['MA_NGANH' => $nganhhang->MA_NGANH,'TEN' => $nganhhang->TEN] ;
    ...
    public function industries_edit(Request $req){
        ...
        return view ("industries/industries_edit",['nganhhang' => $industries, 'loai' => $loai[0]]);
        
    }
    public function industries_update(Request $req) {
        $nganhhang = new Nganhhang(["MA_NGANH"=>$req->MA_NGANH,"TEN"=>$req->TEN]);
        $param = ['MA_NGANH' => $nganhhang->MA_NGANH,'TEN' => $nganhhang->TEN,'ID'=>$req->id];
        $sql = 'UPDATE `nganhhang` Set `TEN` = :TEN,`MA_NGANH` = :MA_NGANH WHERE MA_NGANH = :ID';
